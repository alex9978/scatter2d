...
# ScatterCacheNode
#
# Saves the transforms created by Scatter nodes in an external resource
...
var _local_cache: ScatterCacheResource
var _scene_root: Node
var _scatter_nodes: Dictionary #Key: Scatter, Value: cached version
...
func _ready() -> void:
	...
	if cache_file.is_empty():
		if Engine.is_editor_hint():
			...
			printerr("Scatter error: You load a ScatterCache node with an empty cache file attribute. Outside of the editor, the addon can't set a default value. Please open the scene in the editor and set a default value.")
			return
...
func update_cache() -> void:
	...
	if not _local_cache:
		_local_cache = ScatterCacheResource.new()
	...
	if err != OK:
		printerr("Scatter2D error: Failed to save the cache file. Code: ", err)
...
func restore_cache() -> void:
	...
	for s in _scatter_nodes:
		...
		var transforms = ScatterTransformList.new()
...
func _discover_scatter_nodes(node: Node) -> void:
	if node is Scatter2D and not _scatter_nodes.has(node):
...
func _purge_outdated_nodes() -> void:
	var nodes_to_remove: Array[Scatter2D] = []
