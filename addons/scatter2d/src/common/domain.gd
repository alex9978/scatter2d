...
class DomainShapeInfo:
	...
	func is_point_inside(point: Vector2, local: bool) -> bool:
		var t: Transform2D
	...
	func add(polygon: PackedVector2Array) -> void:
		...
		if Geometry2D.is_polygon_clockwise(polygon):
			...
			if not outer.is_empty():
				print_debug("Scatter error: Replacing polygon's existing outer boundary. This should not happen, please report.")
			outer = polygon
...
func is_point_inside(point: Vector2, local := true) -> bool:
...
func is_point_excluded(point: Vector2, local := true) -> bool:
	for s in negative_shapes:
		if s.is_point_inside(point, local):
			return true
...
func discover_shapes(root_node: Node2D) -> void:
	root = root_node
...
func compute_bounds() -> void:
	...
	var gt: Transform2D = root.get_global_transform().affine_inverse()
...
func _discover_shapes_recursive(node: Node) -> void:
	if node is Scatter2D: # Ignore shapes under nested Scatter nodes
		return
