...
const PRESETS_PATH = "res://addons/scatter2d/presets"
...
func save_preset(scatter_node: Node2D) -> void:
	if not scatter_node:
		return
...
func load_preset(scatter_node: Node2D) -> void:
	if not scatter_node:
		return
...
func load_default(scatter_node: Node2D) -> void:
	_scatter_node = scatter_node
...
func _populate() -> void:
	...
	if not dir:
		print_debug("Scatter2D error: Could not open folder ", PRESETS_PATH)
		return
...
func _on_save_preset(path) -> void:
	...
	preset.clear_output()
	ScatterUtils.set_owner_recursive(preset, preset)
	preset.global_transform.origin = Vector3.ZERO

	var packed_scene = PackedScene.new()
	if packed_scene.pack(preset) != OK:
		print_debug("Scatter2D error: Failed to save preset")
		return

	var err = ResourceSaver.save(packed_scene, path)
	if err:
		print_debug("Scatter2D error: Failed to save preset. Code: ", err)
...
func _on_load_full_preset(path: String) -> void:
	...
	if preset:
		...
		for c in preset.get_children():
			if c is Marker2D or c.name == "ScatterOutput":
				continue
			preset.remove_child(c)
			c.owner = null
			_scatter_node.add_child(c, true)

		ScatterUtils.set_owner_recursive(_scatter_node, get_tree().get_edited_scene_root())
		preset.queue_free()
